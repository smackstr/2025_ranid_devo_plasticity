...
plot1 <- ggplot(data = data.lit, aes(y=delta_density_perliter, x = delta_num_tad, shape = pub_index, fill = species, color = species, group=interaction(pub_index, species))) + 
  ...
  scale_x_continuous(name = paste("\u0394", "number of tadpoles", sep=" "), limits = c(0,300))


plot2 <- ggplot(data = data.lit, aes(y=delta_density_perliter, x = num_tad, shape = pub_index, fill = species, color = species, group=interaction(pub_index, species))) + 
  geom_line(position = position_jitterdodge(jitter.width = 0,
                                            jitter.height = 0)) +
  geom_point(position = position_jitterdodge(jitter.width = 0,
                                             jitter.height = 0), size = 5, alpha = 1) + 
...
# create panel plot with relative and absolute tadpole perspectives
